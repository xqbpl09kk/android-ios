 ...
 * *
 * * 开源库， 以后还是自己验证， 维护更好， 一定要经过自己 debug ， 验证过的 简单库， 或者业界
 * * 标准库， 不标准的 整合性的 小库 不能用！！！！
 */
@SuppressLint("ClickableViewAccessibility")
class UIRefreshControl(context: Context, attrs: AttributeSet?) : ViewGroup(context, attrs) {
	...
	private fun updateListenerCallBack() {
		val distance = mCurrentTargetOffsetTop + (mHeadViewContainer?.height ?: 0)
		if (mListener != null) {
	...
	init {
		...
		mHeaderViewWidth = display.width.toInt()
		mFooterViewWidth = display.width.toInt()
	...
	override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int,
		...
		var distance = mCurrentTargetOffsetTop + (mHeadViewContainer?.measuredHeight ?: 0)
		...
		val headViewWidth = mHeadViewContainer?.measuredWidth ?: 0
		val headViewHeight = mHeadViewContainer?.measuredHeight ?: 0
		...
		val footViewWidth = mFooterViewContainer?.measuredWidth ?: 0
		val footViewHeight = mFooterViewContainer?.measuredHeight ?: 0
	...
	public override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
		...
		if (!mUsingCustomStart && !mOriginalOffsetCalculated) {
			...
			mOriginalOffsetTop = -(mHeadViewContainer?.measuredHeight ?: 0)
	...
	val isChildScrollToTop: Boolean
		get() {
			if (Build.VERSION.SDK_INT < 14) {
				val absListView = mTarget
				if (absListView is AbsListView) {
	...
	val isChildScrollToBottom: Boolean
		get() {
			...
			if (mTarget is RecyclerView) {
				...
				} else if (layoutManager is com.tonicartos.superslim.LayoutManager) {
				...
				val count = absListView?.adapter?.count ?: 0
				val fristPos = absListView?.firstVisiblePosition ?: 0
				if (fristPos == 0 && (absListView?.getChildAt(0)?.top ?: 0) >= (absListView
						?.paddingTop ?: 0)) {
					return false
				}
				val lastPos = absListView?.lastVisiblePosition ?: 0
		...
		when (action) {
			MotionEvent.ACTION_DOWN -> {
				setTargetOffsetTopAndBottom(
						mOriginalOffsetTop - (mHeadViewContainer?.top ?: 0), true)// 恢复HeaderView的初始位置
	...
	fun resetTargetLayout() {
		...
		val childWidth = (child?.width ?: 0) - paddingLeft
		-paddingRight
		val childHeight = (child?.height ?: 0) - paddingTop
		-paddingBottom
	...
	private val mAnimateToCorrectPosition = object : Animation() {
		public override fun applyTransformation(interpolatedTime: Float, t: Transformation) {
			...
			val offset = targetTop - (mHeadViewContainer?.top ?: 0 )
	...
	private inner class HeadViewContainer(context: Context) : RelativeLayout(context) {
		...
		public override fun onAnimationEnd() {
			...
			// annimation 已经结束了
			if (mListener != null) {
				// MARK: 就是 一个 垃圾补丁， Kotlin 版本 一定要给一个 动画参数， 这个时候已经挂了！！
				mListener?.onAnimationEnd(animation ?: AlphaAnimation(0f, 0f))
			}
		}
	}
	...
	inner class CircleProgressView : View, Runnable {
		...
		private var _width: Int = 0// view的高度
		private var _height: Int = 0// view的宽度
		...
		override fun onDraw(canvas: Canvas) {
			super.onDraw(canvas)
			canvas.drawArc(bgUIEdgeInsetsMake, 0f, 360f, false, createBgPaint())
			...
			canvas.drawArc(ovalUIEdgeInsetsMake, startAngle.toFloat(), swipeAngle.toFloat(), false,
					createPaint())
		}

		private val bgUIEdgeInsetsMake: RectF
			get() {
				_width = width
				_height = height
				if (bgRect == null) {
					val offset = (density * 2).toInt()
					bgRect = RectF(offset.toFloat(), offset.toFloat(), (_width - offset).toFloat(), (_height - offset).toFloat())
				}
				return bgRect!!
			}

		private val ovalUIEdgeInsetsMake: RectF
			get() {
				_width = width
				_height = height
				if (ovalRect == null) {
					val offset = (density * 8).toInt()
					ovalRect = RectF(offset.toFloat(), offset.toFloat(), (_width - offset).toFloat(), (_height - offset).toFloat())
				}
				return ovalRect!!
			}
	...
	companion object {
		...
		private val LOG_TAG = "UIRefreshControl"
